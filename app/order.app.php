...
{ 
	...
	function checkout()
    {
        ...
        {
			$mod_cart =& m('cart');
			$g_total =   $mod_cart->get_sumtotal(SESS_ID);

			$this->assign('g_total', $g_total);
			$this->display('order_checkout.html');	
		...
		else
		{
			...
			$mem = $member_mod->find($get_id );
			$mem =current($mem);


			$shipping_id = $_POST['shipping_id'];
			$rowship =  $shipping_mod->find($shipping_id);
			$rowship =current($rowship);
			$data2 = array(
						...
						'seller_id' => '1',
						'seller_name'  => '9999999',
						'buyer_id' => $mem['user_id'],	
						'buyer_name'  => $_POST['consignee'], 
						'buyer_email' => $mem['email'],	
						'status'  => '20', 
						'add_time'      => gmtime(),
						'payment_id'  => '1',
						'payment_name' => 'โอนเงิน',
						'payment_code' => 'bank transfer',
						'goods_amount' => $_POST['total'],
			 
						'order_amount' => $_POST['total'],
			...
			$data3 = array(
						...
						'shipping_id' => $rowship['shipping_id'],
						'shipping_name' => $rowship['shipping_name'],
					);
			...
			$items = $mod_cart->find(array(
				'conditions'    => $conditions,
				 
			));
			foreach ($items  as $key2 => $valluer)
			{
				$data5 = array(
			                'order_id' => $order_id,
							 
							'goods_name'  	 => $items[$key2]['goods_name'],
							'goods_name_thai'  	 => $items[$key2]['goods_name_thai'],
							'price' => $items[$key2]['price'],
							'price_thai' => $items[$key2]['price_thai'],
							'quantity' => $items[$key2]['quantity'],
							'color' => $items[$key2]['color'],
							'size' => $items[$key2]['size'],
							'is_valid' =>'1',
							'goods_image'  =>$items[$key2]['goods_image'],
							'link'  =>$items[$key2]['adlink'],
			            );
	
				$order_goods_mod->add($data5);
			}
			$mailtemplate_mod =& m('mailtemplate');
			$mailtemplate_mod->_mail_touser($_POST['consignee'],$_POST['total'],$mem['email']);
            $mod_cart->drop("session_id='" . SESS_ID . "'");
			$this->show_message('สั่งซื้อสินค้า เสร็จสิ้น' , 'go_back', 'index.php?app=member&act=order');
