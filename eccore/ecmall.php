...
{
    $page_lang = "th";
	if(!isset($_COOKIE['sess_lang']))  
	{
		
		setcookie("sess_lang",$page_lang ,time() + (86400 * 30), "/");  	
	}
	else
	{
		$page_lang = $_COOKIE['sess_lang'];
	}
	  
    return ROOT_PATH . '/languages/' . LANG .  '/' . $file .'_'.$page_lang .'.lang.php';
...
function price_format2($price, $price_format = NULL)
{
    if (empty($price)) $price = '0.00';
