...
function redirect () {
	...
	$.getJSON("index.php?app=calculatecycle&act=getalluser",{'file_id':SITE_URL_ID},function(result){						        
									if(result.retval)
									{
...
function alluser (result) {
	...
	for (i = 0; i < cities.length; i++) {
			...
			setTimeout(readcom(cities[i],round,date_et,date_st,round_id,pay_date), 1000 );
...
function readcom (id,round,date_et,date_st,round_id,pay_date) {
			...
			$.ajax({type:"POST",url:"index.php?app=calculatecycle&act=makecaluser",data:{'file_id':SITE_URL_ID,'id':id,'date_et':date_et,'date_st':date_st,'round_id':round_id,'round':round,'pay_date':pay_date}, success: function(result){
