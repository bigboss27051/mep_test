...
class BackendApp extends ECBaseApp
{
    ...
    function _do_login($user_id)
    {
        ...
        $user_info = $mod_user->get(array(
            'conditions' => $user_id,
            'join'       => 'manage_mall',
            'fields'     => 'this.user_id, user_name, reg_time, last_login, last_ip, privs,type_zero'
        ));
		
		if ($user_info['type_zero'] != 1)
        {
    ...
    function _run_action()
    {
		...
		if ($this->visitor->get('type_zero') != 1)
        {
			$this->show_warning('not_admin');
			return  ;
		}
...
class SoapbaseApp extends ECBaseApp {

	function __construct()
    {
        $this->SoapbaseApp();
    }
    function SoapbaseApp()
    {
        Lang::load(lang_file('admin/common'));
        Lang::load(lang_file('admin/' . APP));
        parent::__construct();
