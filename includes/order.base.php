...
class BaseOrder extends Object
{
	...
	function _gen_order_sn()
    {
		...
		if($sn_row)
		{
			$sn_row = current($sn_row);
		
		
			$code =explode('-',$sn_row['order_sn']); ;
			$str_code =$code[1] + 1 ; 
		}
		else
		{
		...
		switch (strlen($str_code)) {
			  case 1:
			      $str_code ="0000000" . $str_code;
			    break;
			  case 2:
			    $str_code ="000000" . $str_code;
			    break;
			  case 3:
			   	$str_code ="00000" . $str_code;
			    break;
			  case 4:
			   	$str_code ="0000" . $str_code;
			    break;
			  case 5:
    ...
    function _handle_order_info($goods_info, $post)
    {
		...
		$emaill      =  $visitor->get('email');
		if($post['cust_userid']) 
		{
			 $user_id =$post['user_id'];
			 $user_name =$post['cust_userid'];
			 
		}
		if($post['cust_regis']) 
		{
			 $user_id =$goods_info['user_id'];
			 $user_name =$goods_info['user_name'];
		}
		if($post['cust_app']) 
		{
			 $user_id =$goods_info['user_id'];
			 $user_name =$goods_info['user_name'];
			 $emaill  =$goods_info['email'];
		}
    ...
    function _handle_consignee_info($goods_info, $post)
    {
        ...
        /* 计算配送费用 */
        $shipping_model =& m('shipping');
        $shipping_info  = $shipping_model->get("shipping_id={$consignee_info['shipping_id']}   AND enabled=1");
