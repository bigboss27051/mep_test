...
class DefaultPassportUser extends BasePassportUser
{
	...
	function register2($user_name, $password, $email  ,$fidv,$phone,$fb_pic,$local_data = array())
    {
        if (!$this->check_username($user_name))
        {
            return false;
        ...
        $local_data['email']        = $email;
        $local_data['reg_time']     = gmtime();
		$local_data['phone_tel']     = $phone;
		$local_data['phone_mob']     = $phone;
		$local_data['facebook_id']     = $fidv;
		$local_data['fb_pic']  =$fb_pic;
	...
	function edit2($user_id,  $items, $force = false)
    {
	...
	function auth_face($user_name, $password)
    {
        $info = $this->get($user_name, true);
        if (!$info)
        {
	...
	function get_face($flag, $is_name = false)
    {
        if ($is_name)
        {
            $conditions = "facebook_id='{$flag}'";
        }
