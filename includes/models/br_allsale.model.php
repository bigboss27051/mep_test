...
class Br_allsaleModel extends BaseModel
{
    var $table  = 'br_allsale';
    var $prikey = 'allsale_id';
    var $_name  = 'br_allsale';
     

	function _get_allsale_total_in($stid)
	{
		return $this->getOne("SELECT SUM(value) as total FROM " . $this->table . "  WHERE type =1 and  status =1 and member_id =" . $stid );
	} 
	function _get_allsale_total_out($stid)
	{
